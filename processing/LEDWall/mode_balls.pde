...
void setupParticles() {
  ...
  for (int i = 0; i < particles.length; i++) {
    ...
    particles[i] = new Particle(loc, i % 4);
  }
...
class Attractor extends VerletParticle2D {
  ...
  Attractor (Vec2D loc) {
    ...
    physics.addBehavior(new AttractionBehavior(this, buffer.width / 2, 0.025));
  }
...
class Particle extends VerletParticle2D {
  ...
  void updateit() {
    if (lifespan < 0) {
      ...
      float push = map(audio.averageSpecs[spectrum].value, 0, 100, 0.00025, 0.1);
      f = f.scale(push );
      f.limit(2);
      addForce(f);
    }
